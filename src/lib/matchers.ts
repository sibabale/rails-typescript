...
import { RailsDefaultError } from "../models/errors/railsdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | RailsDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | RailsDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new RailsDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new RailsDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
